    ...
    try:
        ...
        if uploaded_file.name.endswith('.csv'):
            ...
            for i, line in enumerate(lines):
                ...
                matches = sum(1 for k in ['ID', 'INVOICE', 'NAME', 'DATE', 'PRICE', 'AMOUNT', 'CATEGORY', 'COUNTRY', 'TOTAL', 'COUNT'] if k in cleaned_line)
                if matches >= 1:
                    header_index = i
                    break
        ...
        # Base rule classifications (Fallback mode context matching)
        ...
        # --- ABSOLUTE SAFE FALLBACK PROTECTION ---
        if not numeric_cols:
            potential_nums = [col for col in df.columns if df[col].dtype in ['int64', 'float64']]
            numeric_cols = [potential_nums[0]] if potential_nums else []
            
        if not category_cols:
            potential_cats = [col for col in df.columns if df[col].dtype == 'object']
            category_cols = [potential_cats[0]] if potential_cats else [df.columns[0]]
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            ...
            for row in range(2, data_sheet.max_row + 1):
                for col in range(1, data_sheet.max_column + 1):
                    ...
                    if isinstance(cell.value, (int, float)) and not any(k in data_sheet.cell(row=1, column=col).value.lower() for k in ['phone', 'mobile', 'id', 'code']):
                        ...
                        if any(k in data_sheet.cell(row=1, column=col).value.lower() for k in ['phone', 'mobile', 'code']):
                            cell.number_format = '@'
            ...
            if category_cols and numeric_cols:
                ...
                for i, row_data in summary_data.iterrows():
                    ...
                    val_cell.number_format = '#,##0.00' if val_cell.value % 1 != 0 else '#,##0'
                ...
                if category_cols:
                    ...
                    dashboard_sheet["A4"] = category_cols[0]
                    dashboard_sheet["B4"] = "Record Frequency Count"
                    dashboard_sheet["A4"].font = header_font
                    dashboard_sheet["A4"].fill = header_fill
                    dashboard_sheet["B4"].font = header_font
                    dashboard_sheet["B4"].fill = header_fill
                    ...
                    dashboard_sheet["A2"] = "Upload a structured descriptive data sheet to automatically map dashboard chart logs."
